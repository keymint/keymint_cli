...
While all entry points in this package start with ``keymint_cli.`` other
distributions might define entry points with a different prefix.
Those need to be declared using this group name.
"""
EXTENSION_POINT_GROUP_NAME = 'keymint_cli.extension_point'
...
def get_all_entry_points():
    """
    Get all entry points related to ``keymint_cli`` and any of its extensions.
